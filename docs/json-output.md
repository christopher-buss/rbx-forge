...
Failures: `compile_timeout` (exit 1) after `--timeout <ms>` (default 300000),
with `details.building` and `details.timeoutMs`; `service_failed` (exit 1) when
the compiler stops during the wait; `not_running` (exit 3) when no session runs
or it stops.
