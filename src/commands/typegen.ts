...
export async function action(commandOptions: TypegenOptions = {}): Promise<void> {
	...
	logger.info(ansis.bold("→ Generating service types"));

	const startTime = performance.now();
	const spinner = yoctoSpinner({ text: "Fetching Rojo sourcemap..." }).start();

	const projectPath = commandOptions.project ?? config.rojoProjectPath;
	const rojoSourceMap = await getRojoSourceMap(config, projectPath);
	...
	spinner.success(`Types generated (${ansis.dim(fileInfo)})`);
}
