 ...
 * `down`). First it closes the session's Studio, unless `--keep-studio`;
 * a Studio forge cannot close is reported in `studio`, and the session
 * still stops. It reports `stopped` only once the session's supervisor has
