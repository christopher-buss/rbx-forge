...
/**
 * The config values the flags set, as the CLI would pass them.
 *
 * @param flags - The parsed flags.
 * @returns `syncback.runOnStart` for `--syncback`.
 */
function configLayer(flags: FlagValues): { syncback?: { runOnStart: boolean } } {
	return flags["syncback"] === true ? { syncback: { runOnStart: true } } : {};
}
...
function startCommand({
	...
	return {
		...
		memory,
		reporter,
		result: runStartAsync(context, { config: configLayer(flags), flags }),
		signals,
		studioLauncher,
	};
}
