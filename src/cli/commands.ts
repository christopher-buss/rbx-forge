...
export const COMMANDS: ReadonlyArray<CommandDefinition> = [
	...
	},
	{
		...
		run: runUpAsync,
		summary:
			"Start the dev session in the background, as start does, and return once each of its services (Rojo, the compiler) is ready. Reports the running session if there is one.",
	},
	{
		name: "status",
		flags: STATUS_FLAGS,
